...
function getDetachedAssetToken(): string {
  try {
...
function getDetachedAssetUrl(path: string): string {
  try {
    const url = new URL(path, window.location.origin);
    const token = getDetachedAssetToken();
  ...
  <script>
    ...
    async function getMicroBreakSoundSource(fileName) {
      return fileName
        ...
        : ${JSON.stringify(getDetachedAssetUrl('/plugins/pinch/audio/correct.mp3'))};
    }
...
async function handleDetachedFocusRequest(_event: unknown, request?: DetachedFocusRequest): Promise<unknown> {
  ...
  switch (request.type) {
    ...
    case 'show-micro-break-dialog': {
      ...
      const awardImageUrl = getDetachedAssetUrl('/plugins/pinch/images/award.png');
      const teaImageUrl = getDetachedAssetUrl('/plugins/pinch/images/tea.png');
      const randomImageUrl = getDetachedAssetUrl('/plugins/pinch/images/random.png');
