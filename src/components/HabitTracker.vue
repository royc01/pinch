...
const calculateCurrentStreak = (habit: Habit, startDate?: Date) => {
  // 创建缓存键
  const cacheKey = `${habit.id}-${startDate ? startDate.getTime() : 'none'}-${Date.now() - (Date.now() % 86400000)}`; // 包含日期但忽略时间
  
  // 检查缓存（每日重置）
  const cached = streakCache.get(cacheKey);
  if (cached && (Date.now() - cached.timestamp) < 86400000) { // 一天内有效
...
const getWeeklyCompletionStatus = (habit: Habit) => {
  ...
  // 创建缓存键
  const cacheKey = `${habit.id}-weeklyStatus-${getWeekStart(new Date()).toISOString().split('T')[0]}`; // 按周缓存
  ...
  // 计算本周已完成的打卡次数 - 使用reduce替代filter+length提高性能
...
const calculateLongestStreak = (habit: Habit) => {
  // 创建缓存键
  const cacheKey = `${habit.id}-longestStreak-${Date.now() - (Date.now() % 86400000)}`; // 包含日期但忽略时间
...
const calculateTotalCompletionRate = (habit: Habit) => {
  // 创建缓存键
  const cacheKey = `${habit.id}-completionRate-${Date.now() - (Date.now() % 86400000)}`; // 包含日期但忽略时间
...
const totalCompletionsCount = computed(() => {
  let total = 0;
  for (const habit of habits.value) {
    // 使用reduce替代filter+length，一次遍历完成计算
