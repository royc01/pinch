...
export function taskToCRDT(task: Task, nodeId: string = 'db'): CRDTTask {
  ...
  return {
    ...
    urgent: baseField(task.urgent === true, updatedAt),
...
export function crdtToTask(crdtTask: CRDTTask): Task {
  return {
    id: crdtTask.id,
    taskId: crdtTask.metadata.taskId,
    ...
    urgent: crdtTask.urgent?.value === true,
    ...
    sourceBlockId: crdtTask.metadata.sourceBlockId,
...
export class CRDTTaskRepository {
  ...
  private ensureRemoteTaskTs(crdtTask: CRDTTask): CRDTTask {
    ...
    return {
      ...
      urgent: withTs(guardedTask.urgent || { value: false, ts: guardedTask.updatedAt, node: this.nodeId }),
